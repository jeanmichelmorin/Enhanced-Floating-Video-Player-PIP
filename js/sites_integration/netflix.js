window.addEventListener("load", function(event) {
	...
	let intervalId = setInterval( () => { // Since the way Netflix works doesn't really refresh the page (Web App), we need to check actively every X milliseconds if the user is on the right page to display the button.

		if (window.location.href.indexOf("netflix.com/watch/") > -1 && !document.getElementsByClassName("pip-button")[0]) {
			...
			console.log(GLOBAL_msgPrefix + "Looking to place my PIP...");

					if (!document.getElementsByClassName("pip-button")[0]) {

						try {

							let PATH_OF_PARENT_TO_PLACE_BUTTON = ".PlayerControlsNeo__button-control-row";
							let POSITION = 'beforeend';
							let INJECT_HTML_BUTTON = '<button class="pip-button touchable PlayerControls--control-element nfp-button-control default-control-button" tabindex="0" role="button" aria-label="Picture-In-Picture (PIP)">'+svg+'</button>';
							let BUTTON_TARGET = ".pip-button";

							setupPipButton(PATH_OF_PARENT_TO_PLACE_BUTTON, POSITION, INJECT_HTML_BUTTON, BUTTON_TARGET);
							
							//clearInterval(intervalId);
							console.log(GLOBAL_msgPrefix + "Found the spot to place my PIP button.");
